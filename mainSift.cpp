...
extern "C"{
	void free_RetStruct(RetStruct* p);
	int read_image(char* p);
	RetStruct* img_sift_process(int height1, int width1, uchar* data1);
...
RetStruct* sift_process(int height1, int width1, uchar* data1,int height2, int width2, uchar* data2,int img_number)
{
  ...
  /*std::ofstream outfile1 ("21.txt");
  outfile1 << limg1 << "123" <<std::endl <<std::endl<< std::endl <<std::endl<< std::endl <<std::endl<< std::endl <<std::endl;
  std::ofstream outfile2 ("22.txt");
  outfile2 << rimg1 << "123"  <<std::endl <<std::endl<< std::endl <<std::endl<< std::endl <<std::endl<< std::endl <<std::endl;
  */
  //cv::Mat rimg(640,720, CV_32FC1, cv::Scalar(0, 255, 255));
  //cv::Mat rimg(640, 720, CV_32FC1, cv::Scalar(0, 255, 255));
  //cv::Mat limg;
  //resize(limg1,limg , rimg.size(), 0, 0);
  //resize(rimg1,rimg , rimg.size(), 0, 0);
  //cv::Mat rimg;
  //cv::Mat limg;
  ...
  unsigned int w = rimg.cols;
  unsigned int h = rimg.rows;
  std::cout << "Image size = (" << w << "," << h << ")" << std::endl;
  ...
  InitCuda(devNum);
  CudaImage img1, img2;
  img1.Allocate(limg.cols, limg.rows, iAlignUp(limg.cols, 128), false, NULL, (float*)limg.data);
  img2.Allocate(rimg.cols, rimg.rows, iAlignUp(rimg.cols, 128), false, NULL, (float*)rimg.data);
  img1.Download();
  img2.Download();
  ...
  // A bit of benchmarking
  //for (int thresh1=1.00f;thresh1<=4.01f;thresh1+=0.50f) {
  float *memoryTmp = AllocSiftTempMemory(1280,1280, 5, false);
  ExtractSift(siftData1, img1, 5, initBlur, thresh, 0.0f, false, memoryTmp);
  ExtractSift(siftData2, img2, 5, initBlur, thresh, 0.0f, false, memoryTmp);
  FreeSiftTempMemory(memoryTmp);
  ...
  //float homography[9];
  //int numMatches;
  //FindHomography(siftData1, homography, &numMatches, 10000, 0.00f, 1000.f, 5.0);
  //int numFit = ImproveHomography(siftData1, homography, 5, 0.00f, 0.80f, 3.0);
  ...
  std::string address3 = ".jpg";
  cv::imwrite(address+std::to_string(img_number)+address3 , limg);
  //MatchAll(siftData1, siftData2, homography);
...
RetStruct* img_sift_process(int height1, int width1, uchar* data1){
  ...
  SiftData siftData1;
  float initBlur = 0.0f;
  float thresh = 0.04f;//(imgSet ? 4.5f : 3.0f);
  InitSiftData(siftData1, 32768, true, true);
  ...
  for(int i=0;i<p->numPts;i++){
        ...
        //std::cout<<p->x_pos[i]<<' '<<p->y_pos[i]<<' '<<p->m_x_pos[i]<<' '<<p->m_y_pos[i]<<' '<<std::endl;
  }
